...
def segmentation2(cropped):
    ...
    oc_distance = [get_distance(oc_cx, od_cy, j, i) for i in range(opening_oc.shape[0]) for j in range(opening_oc.shape[1])]
    oc_distance = np.reshape(oc_distance, opening_oc.shape)

    od_distance = [get_distance(od_cx, od_cy, j, i) for i in range(opening_od.shape[0]) for j in range(opening_od.shape[1])]
...
for i, fichier in enumerate(fichiers):
    ...
    cv2.imwrite(outpath_dossier+'0_crop.png', cropped)
    ...
    cv2.imwrite(outpath_dossier+"oc_hough.jpg", oc_rgb)
    cv2.imwrite(outpath_dossier+"od_hough.jpg", od_rgb)
...
moy_sensitivity_oc = np.mean(results[1::,1].astype('float'))
moy_precision_oc = np.mean(results[1::,2].astype('float'))
moy_fscore_oc = np.mean(results[1::,3].astype('float'))
moy_sensitivity_od = np.mean(results[1::,4].astype('float'))
moy_precision_od = np.mean(results[1::,5].astype('float'))
moy_fscore_od = np.mean(results[1::,6].astype('float'))
